...
export default function PipelinePage() {
  ...
  const loadJobs = useCallback(async () => {
    try {
      ...
      const arr: PipelineJob[] = (
        Array.isArray(data) ? data : (data.keywords ?? [])
      )
  ...
  const activeJobs = jobs.filter((j) =>
    ["researching", "writing", "validating", "publishing"].includes(j.status),
  );
  ...
  function JobRow({ job }: { job: PipelineJob }) {
    ...
    return (
      ...
      >
        ...
        >
          {/* Top row on mobile: chevron + keyword + status */}
          <div
            style={{
              display: "flex",
              alignItems: "center",
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {!isMobile && (
                <div
                  style={{ fontSize: "12px", color: "var(--th-text-muted)" }}
                >
                  {job.site ?? ""}
                  {duration && (
          ...
          {/* Stage progress — full width row on mobile */}
          ...
          {/* Duration — mobile only, below stage */}
          {isMobile && duration && (
            <div style={{ fontSize: "11px", color: "var(--th-text-muted)" }}>
              {duration}
            </div>
          )}
        </div>
          ...
          >
            {detailLoading === job.id ? (
              ...
              >
                <div
                  className="skeleton"
                  style={{ height: "14px", width: "50%", borderRadius: "4px" }}
                />
                <div
                  className="skeleton"
                ...
                <div
                  className="skeleton"
                  style={{ height: "80px", width: "100%", borderRadius: "4px" }}
                />
              </div>
            ) : (
              <div
              ...
              >
                ...
                <div>
                  ...
                  >
                    {["research", "write", "validate", "publish"].map(
                      (stage) => {
                        const progress = (detail?.stageProgress ??
                          job.stageProgress)[stage];
                ...
                <div>
                  ...
                  {(detail?.articleOutput ?? job.articleOutput) && (
                    <div style={{ marginTop: "16px" }}>
                      ...
                      >
                        {
                          (detail?.articleOutput ?? job.articleOutput)?.metadata
                            ?.title
                        }
                      </div>
                      <div
                  ...
                  {!detail?.researchOutput &&
                    !detail?.articleOutput &&
                    !job.researchOutput &&
                    !job.articleOutput && (
                      <div
                        style={{
